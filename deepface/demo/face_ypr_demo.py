...
#shape_predictor_68_face_landmarks.dat 是一个预训练的人脸关键点检测模型，可以用于识别人脸的68个关键点，如眼睛、鼻子、嘴巴等。这个模型可以被用于人脸识别、人脸表情分析、面部姿势估计等领域。它是由dlib库提供的，可以在Python中使用。如果你想使用它，可以在dlib的官方网站上下载。
...
def draw_annotation_box(image, rotation_vector, translation_vector, camera_matrix, dist_coeefs,color=(0, 255, 0), line_width=2):
        """
...
if __name__ == '__main__':
    ...
    for imagePath in range(2):    
        print(f"处理的图片路径为： {imagePath}")
        # Read Image
        im = cv2.imread("W:\\python_code\\deepface\\database\\new_19d64eb2cc63f43ed8dd1e4c159b33da5.jpg")
        size = im.shape
        # 对图像进行缩放的操作
        if size[0] > 700:
            h = size[0] / 3
        ...
        cv2.imwrite('temp\\new_'+  str(uuid.uuid4()).replace('-', '')+".jpg", imgss)
